...
public class StatePanel extends JPanel {
	...
	public StatePanel(int stateSizeX, int stateSizeY, int scale) {
		...
		generateColors();
		initStatePanel(stateSizeX, stateSizeY, scale);
	}
	
	public void initStatePanel(int stateSizeX, int stateSizeY, int scale) {
	...
	public void renderState(int[][] state, int stateSizeX, int stateSizeY) {
		for (int x = 0; x < stateSizeX; x++) {
			for (int y = 0; y < stateSizeY; y++) {
				stateImage.setRGB(x, y, color[state[x][y]].getRGB());
			}
		}
